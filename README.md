...
`data/pause_boundaries_source.csv` is regenerated by `code/05_boundaries.py` when local audio is available. With the verified PCM directory used for this package, the current boundary export recovers all 64 source-condition rows from 16 distinct performances. `data/pause_boundaries_failures.csv` remains part of the workflow so failed recordings are logged rather than silently repaired when another local audio directory is used.

Quality-control note: one nominally separate Arrau/Fischer source pair was found to contain the same performance. The boundary starts agree within 0.20 s, and `code/08_same_performance.py` confirms the match by energy-envelope cross-correlation (`r=0.978`, lag `+0.095 s`). The Fischer row remains in `data/manifest.csv` with a deduplication note, but `load_manifest()` excludes it from the core statistics.
...
- Level sensitivity is quantified, not merely noted: peak normalization alone moves per-recording mean `d_l` over a 1.97 s range. The most affected source is an early transfer with a high noise floor, which is consistent with the detector partly tracking transfer vintage; 16 recordings cannot establish that correlation, so it is flagged as a direction rather than a finding.
