...
`data/pause_boundaries_source.csv` is regenerated by `code/05_boundaries.py` when local audio is available. The current boundary export recovers 64 of 68 source-condition rows; `data/pause_boundaries_failures.csv` logs the remaining difficult case rather than silently repairing it.
