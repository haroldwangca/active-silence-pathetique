...
Quality-control note: an all-pairs screen on detected onset positions flagged exactly one nominally separate Arrau/Fischer source pair. The four boundary starts agree within 0.20 s, against a corpus median pairwise disagreement of 14.70 s by maximum absolute onset difference. `code/08_same_performance.py` confirms the match by energy-envelope cross-correlation (`r=0.978`, lag `+0.095 s`). After removal, the closest remaining pair differs by 1.26 s. The Fischer row remains in `data/manifest.csv` with a deduplication note, but `load_manifest()` excludes it from the core statistics.
...
- Level sensitivity is quantified, not merely noted: peak normalization changes the within-recording P2-P3 difference by 0.25 s on average, ranges from -2.47 to +3.62 s across individual recordings, and moves the paired mean from -0.49 s to -0.24 s.
...
- `d_s` is not treated as a pure performance variable because it mixes acoustics, transfer, score texture, pedaling, and release behavior. The low-energy duration `d_l` is also energy-detector output rather than a direct perceptual boundary: it can reflect string decay, room response, transfer processing, and score-prescribed dynamics as well as performer-controlled silence.
